...
CMD ["npm", "run", "dev", "--workspace=apps/api-gateway"]
...
# to any registry) and to install api-gateway's actual runtime dependencies —
...
RUN npm install --workspace=apps/api-gateway --workspace=packages/shared-types --workspace=packages/shared-utils --omit=dev
