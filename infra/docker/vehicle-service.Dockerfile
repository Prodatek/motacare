...
RUN npm install --workspace=apps/vehicle-service --workspace=packages/shared-types --workspace=packages/shared-utils
...
CMD ["npm", "run", "dev", "--workspace=apps/vehicle-service"]
...
RUN npm run build --workspace=apps/vehicle-service
...
COPY --from=builder /app/apps/vehicle-service/drizzle ./drizzle
