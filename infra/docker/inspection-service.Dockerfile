...
# to any registry) and to install inspection-service's actual runtime dependencies —
...
COPY --from=builder /app/apps/inspection-service/package.json ./apps/inspection-service/

RUN npm install --workspace=apps/inspection-service --workspace=packages/shared-types --workspace=packages/shared-utils --omit=dev
...
COPY --from=builder /app/apps/inspection-service/drizzle ./drizzle
