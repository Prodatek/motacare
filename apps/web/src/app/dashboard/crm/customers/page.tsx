...
import { crmApi, ApiClientError } from '@/lib/api';
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400 flex-wrap">
                    <span className="flex items-center gap-1"><Wrench className="h-3 w-3" />{c.totalFixJobs} fix job{c.totalFixJobs !== 1 ? 's' : ''}</span>
                    {c.totalSpend > 0 && <span>{formatNGN(c.totalSpend)}</span>}
                    {c.lastVisitAt && <span className="flex items-center gap-1"><Clock className="h-3 w-3" />Last {formatDate(c.lastVisitAt)}</span>}
                  </div>
                </div>
              </div>
