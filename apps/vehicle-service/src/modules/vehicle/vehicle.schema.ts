...
export const registerVehicleSchema = z.object({
  ...
  make: z.string().min(1).max(100).transform((v) => v.trim()),
  model: z.string().min(1).max(100).transform((v) => v.trim()),
  year: z
    .coerce.number()
    .int()
  ...
  engineCapacity: z.string().max(20).trim().optional(),
  engineCode: z.string().max(50).trim().optional(),
